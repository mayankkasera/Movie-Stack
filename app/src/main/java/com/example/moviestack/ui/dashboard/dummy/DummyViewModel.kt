...
class DummyViewModel(var movieItemRepositoryI: MovieItemRepositoryI,
                     var trendingRepositoryI: TrendingRepositoryI,
                     var tvShowRepositoryI: TvShowRepositoryI) : ViewModel()  {

    var mutableLiveData: MutableLiveData<DummyState> = MutableLiveData()
    private var compositeDisposable = CompositeDisposable()
    private var state = DummyState()
        set(value) {
            field = value
            publishState(value)
        }
            ...
            Observable.merge(
                ...
                when (it.type) {
                    ...
                    SmallItemList.Type.TRENDING_PERSON -> state =
                        state.copy(
                            personVerticalAdapter = PersonVerticalAdapter(it.results)
                        )
    ...
    private fun publishState(state: DummyState) {
